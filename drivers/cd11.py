...
class cd11_ip(weigh_scale_ip):
    """
    IP driver for the CD11 series weigh scales
    """
    ...
    def __init__(self, hostname, port, timeout=10000):
        ...
        """
        super(cd11_ip, self).__init__(hostname, port, timeout=timeout)
    ...
    def _get_weight(self):
        ...
        if len(value) >= 15 and value[13] != '?':  # and value[14] == 'N':
            # Extract weight and uom name from the returned value
            weight = float(value[:9])
            uom_name = value[10:13].strip()
