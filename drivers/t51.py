...
class t51_ip(weigh_scale_ip):
    """
    IP driver for the T51 series weigh scales
    """
    ...
    def __init__(self, hostname, port, timeout=10000):
        ...
        """
        super(t51_ip, self).__init__(hostname, port, timeout=timeout)
    ...
    def _get_weight(self):
        ...
        if len(value) >= 15 and value[14] != '?':
            ...
            weight = float(value[1:9])
            if value[0] == '-':
                weight *= -1
            uom_name = value[9:14].strip()
