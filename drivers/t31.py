...
class t31_ip(weigh_scale_ip):
    """
    IP driver for the T31 series weigh scales
    """
    ...
    def __init__(self, hostname, port, timeout=10000):
        ...
        """
        super(t31_ip, self).__init__(hostname, port, timeout=timeout)
    ...
    def _get_weight(self):
        ...
        if len(value) >= 16 and value[15] != '?' and value[16:19] == 'NET':
            # Extract weight and uom name from the returned value
            weight = float(value[2:9])
            if value[0] == '-':
                weight *= -1
            uom_name = value[10:15].strip()
